...
void create(Uzond*& arr, short size, short size_of_peopl) {
	...
	for (int i = 0; i < size; i++) {
		arr[i].people = new Users * [size_of_peopl];
	}
}
...
void add(Uzond*& program, short* size, short* size_of_peopl, vector<string> arr_name, vector<string> arr_suname, vector<string>arr_of_name_urzant)
{
	...
	case (117):
		...
		for (int i = 0; i < *size - 1; i++) {
			...
			for (int j = 0; j < *size_of_peopl; j++) {
				program_n[i].people[j] = program[i].people[j];
			}
			program_n[i].Name = program[i].Name;
			program_n[i].Numer = program[i].Numer;
		}
		program_n[*size - 1].people = new Users * [*size_of_peopl];
		for (int j = 0; j < *size_of_peopl; j++) {
			program_n[*size - 1].people[j] = new Users;
	...
	case (111):
		...
		for (int j = 0; j < *size; j++) {
			...
			for (int i = 0; i < *size_of_peopl - 1; i++) {
				program_n[j].people[i] = program[j].people[i];
			}
			program_n[j].people[*size_of_peopl - 1] = new Users;
			...
			program_n[j].Name = program[j].Name;
			program_n[j].Numer = program[j].Numer;
		}
		delete[] program;
		program = program_n;
		break;
...
void dell(Uzond*& program, short* size, short* size_of_peopl) {
	...
	case (111):
	{
		...
		for (int i = 0; i < *size; i++) {
			Users** new_people = new Users * [*size_of_peopl - 1];
			for (int j = 0, k = 0; j < *size_of_peopl; j++) {
				if (j != num - 1) {
					new_people[k] = program[i].people[j];
					k++;
				}
			}
			delete[] program[i].people;
			program[i].people = new_people;
		}
		(*size_of_peopl)--;
		break;
	}
